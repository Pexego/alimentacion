    ...
    def _product_qty_available(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if line.move_id.state != 'done':
                res[line.id] = self.pool.get('product.product').browse(cr, uid, line.product_id.id, context=c).qty_available + self.pool.get('product.uom')._compute_qty(cr, uid, line.move_id.product_uom.id, line.move_id.product_qty, line.product_id.uom_id.id)
            else:

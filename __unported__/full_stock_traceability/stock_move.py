...
class stock_move(osv.osv):
        ...
        for move in self.browse(cr, uid, ids):
            ...
            if move.location_id.name == 'Suppliers':
                if move.picking_id and move.picking_id.partner_id:
                    res[move.id] = move.picking_id.partner_id.name
        return res
    ...
    def onchange_product_id(self, cr, uid, ids, prod_id=False, loc_id=False, loc_dest_id=False, partner_id=False, prod_qty=0.0, prodlot_id=False):
        ...
        parent_result = super(stock_move, self).onchange_product_id(cr, uid, ids, prod_id, loc_id, loc_dest_id, partner_id)

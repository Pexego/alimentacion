...
class stock_move(osv.osv):
    ...
    def _default_location_destination(self, cr, uid, context=None):
        ...
        if context.get('address_out_id', False):
            property_out = self.pool.get('res.partner').browse(cr, uid, context['address_out_id'], context).partner_id.property_stock_customer
            return property_out and property_out.id or False
        if context.get('address_return_id', False):
            property_out = self.pool.get('res.partner').browse(cr, uid, context['address_return_id'], context).partner_id.property_stock_returns
            return property_out and property_out.id or False
    ...
    def _default_location_source(self, cr, uid, context=None):
        ...
        if context.get('customer_address_id', False):
            part_obj_add = self.pool.get('res.partner').browse(cr, uid, context['customer_address_id'], context=context)
            if part_obj_add.partner_id:

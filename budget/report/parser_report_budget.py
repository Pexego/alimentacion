...
class parser_report_budget(report_sxw.rml_parse):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if version.budget_id.budget_item_id and version.budget_id.budget_item_id.children_ids:
            for item in version.budget_id.budget_item_id.children_ids:
                where_date = ''
                where_clause_args = ''
                prev_amount = 0.0
                real_amount = 0.0
              ...
              """ + where_date + """""", where_clause_args)
                ...
                item.jan_amount_real = self._get_real_amount_by_to_date(1, todate, [tuple(child_ids)])
                item.feb_amount_real = self._get_real_amount_by_to_date(2, todate, [tuple(child_ids)])
                item.mar_amount_real = self._get_real_amount_by_to_date(3, todate, [tuple(child_ids)])
                item.apr_amount_real = self._get_real_amount_by_to_date(4, todate, [tuple(child_ids)])
                item.may_amount_real = self._get_real_amount_by_to_date(5, todate, [tuple(child_ids)])
                item.jun_amount_real = self._get_real_amount_by_to_date(6, todate, [tuple(child_ids)])
                item.jul_amount_real = self._get_real_amount_by_to_date(7, todate, [tuple(child_ids)])
                item.aug_amount_real = self._get_real_amount_by_to_date(8, todate, [tuple(child_ids)])
                item.sep_amount_real = self._get_real_amount_by_to_date(9, todate, [tuple(child_ids)])
                item.oct_amount_real = self._get_real_amount_by_to_date(10, todate, [tuple(child_ids)])
                item.nov_amount_real = self._get_real_amount_by_to_date(11, todate, [tuple(child_ids)])
                item.dec_amount_real = self._get_real_amount_by_to_date(12, todate, [tuple(child_ids)])
                item.prev_amount = prev_amount
                item.real_amount = real_amount

                item.jandiff = float(round(((item.jan_amount_real - item.jan_amount) / item.prev_amount) * 100, 2))
                item.febdiff = float(round(((item.feb_amount_real - item.feb_amount) / item.prev_amount) * 100, 2))
                item.mardiff = float(round(((item.mar_amount_real - item.mar_amount) / item.prev_amount) * 100, 2))
                item.aprdiff = float(round(((item.apr_amount_real - item.apr_amount) / item.prev_amount) * 100, 2))
                item.maydiff = float(round(((item.may_amount_real - item.may_amount) / item.prev_amount) * 100, 2))
                item.jundiff = float(round(((item.jun_amount_real - item.jun_amount) / item.prev_amount) * 100, 2))
                item.juldiff = float(round(((item.jul_amount_real - item.jul_amount) / item.prev_amount) * 100, 2))
                item.augdiff = float(round(((item.aug_amount_real - item.aug_amount) / item.prev_amount) * 100, 2))
                item.sepdiff = float(round(((item.sep_amount_real - item.sep_amount) / item.prev_amount) * 100, 2))
                item.octdiff = float(round(((item.oct_amount_real - item.oct_amount) / item.prev_amount) * 100, 2))
                item.novdiff = float(round(((item.nov_amount_real - item.nov_amount) / item.prev_amount) * 100, 2))
                item.decdiff = float(round(((item.dec_amount_real - item.dec_amount) / item.prev_amount) * 100, 2))
    ...
    def get_lines(self, data, item, account):
        ...
        if lines:
            for line in self.pool.get('budget.line').browse(self.cr, self.uid, lines):
                line.name = line.product_id and line.product_id.name or line.name

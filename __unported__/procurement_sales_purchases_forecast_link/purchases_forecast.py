            ...
            if cur.purchases_forecast_lines:
                for month in range(0,12):
                    ...
                    for line in cur.purchases_forecast_lines:
                        ...
                        year = cur.date[:4]
                        month = str(month+1)
                        first_day, last_day = calendar.monthrange(int(time.strftime('%Y')),int(month))
                        complet_last_date = year+"-"+month+"-"+str(last_day)
                        complet_first_date = year+"-"+month+"-"+str(first_day)

                        if periods[month].get(line.product_id.id, False):
                            periods[month][line.product_id.id][0] = periods[month][line.product_id.id][0] + eval('o.' + (months[month] + '_qty'),{'o': line})
                        else:
                            ...
                            if purchases_lines:
                                for pline in self.pool.get('purchase.order.line').browse(cr, uid, puchases_lines):
                                    qty += pline.product_qty
                                    
                            periods[month][line.product_id.id].append(eval('o.' + (months[month] + '_qty'),{'o': line}), qty)
                        ...
                        if period:
                            ...
                            for product in periods[period]:
                                new_id = proc_obj.create(cr, uid, {
                                                                'product_id': product,
                                                                'to_procure': periods[period][product][0],
                                                                'planned_outgoing': periods[period][product][1],
